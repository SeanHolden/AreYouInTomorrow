...
function whosInOnThisDay(When, User, dateString, callback){
  ...
  When.findAll({where:{date:date},include:[User]}).success(function(whens){
    ...
    for(var i=0;i<whens.length;i++){
      var user = whens[i].user.dataValues.firstName +' '+ whens[i].user.dataValues.lastName;
...
function destroyAllWhens(whens, callback){
  for(i in whens){
    whens[i].destroy();
  };
  callback();
...
function destroyUser(user, callback){
  user.destroy()
  .success(function(){
    var msg = 'User successfully deleted from database.';
    callback(msg);
  })
  .error(function(){
    var msg = 'Something went wrong. User was found but not deleted.';
    callback(msg);
  });
}
...
module.exports = { 
  ...
  destroyAllWhens : destroyAllWhens,
